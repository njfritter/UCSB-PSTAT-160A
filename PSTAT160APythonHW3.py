...
def part_b():
    print("\nPart B: Simulating one path of Xn from n = 0  to infinity\n")
...
def part_d(sim_time_steps): 
    print("\nPart D: Estimating probabilities that all songs are heard at time n = 5, 6, 7, .... using values generated from Part C\n")
    # Now to find probabilities
    num_sim = 10000
    n = 5
    probs = []
    while n < 21:
        num_times_correct = sim_time_steps.count(n)
...
def part_e(probabilities):
    ...
    plt.xlabel('Number of time steps to play all songs')
